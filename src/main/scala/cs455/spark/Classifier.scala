...
class Classifier(data: DataFrame) {
  ...
  def classify(alg : String, output : String, trainSplit : Double, testSplit : Double, classes : Int, features : Int, toHDFS : Boolean): Double = {
...
//    train.show(false)
  ...
  def multilayer(layers : Array[Int], train : Dataset[Row], test : Dataset[Row], output : String): Double = {
    ...
    println(evaluate.evaluate(prediction))
...
//    conf.set("fs.defaultFS", "hdfs://juneau:4921")
